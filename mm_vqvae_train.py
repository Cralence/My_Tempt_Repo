...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        "--base",
        nargs="*",
        metavar="music_motion_vae.yaml",
        help="paths to base configs. Loaded from left-to-right. "
             "Parameters can be overwritten or added with command-line options of the form `--key value`.",
        default=['./configs/local_run.yaml'],
        #default=['/nobackup/users/yiningh/yh/music_dance/music_motion_diffusion/configs/mm_vqvae.yaml']
    )
    parser.add_argument(
        "-t",
        "--train",
...
def music_motion_collate(batch):
    notnone_batches = [b for b in batch if b is not None]
    adapted_batch = {
        "motion": torch.stack([torch.tensor(b['motion']).float() for b in notnone_batches]),
        "waveform": torch.stack([b['waveform'] for b in notnone_batches])
    }
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, validation=None, test=None,
        ...
        super().__init__()
        self.collate = music_motion_collate
...
if __name__ == "__main__":
    ...
    try:
        ...
        default_callbacks_cfg = {
            "setup_callback": {
                "target": "mm_vqvae_train.SetupCallback",
                "params": {
                    "resume": opt.resume,
                    "now": now,
                    "logdir": logdir,
            ...
            "learning_rate_logger": {
                "target": "mm_vqvae_train.LearningRateMonitor",
                "params": {

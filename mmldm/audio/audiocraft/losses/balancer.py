...
class Balancer:
    ...
    def backward(self, losses: tp.Dict[str, torch.Tensor], input: torch.Tensor) -> Tuple[Tensor, Tensor]:
        ...
        return effective_loss, out_grad


class MusicMotionBalancer:
    """Loss balancer.
    ...
    def backward(self, losses: tp.Dict[str, torch.Tensor], music_recon: torch.Tensor, motion_recon: torch.Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        """Compute the backward and return the effective train loss, e.g. the loss obtained from
        computing the effective weights. If `balance_grads` is True, the effective weights
        are the one that needs to be applied to each gradient to respect the desired relative
        scale of gradients coming from each loss.
        ...
        for name, loss in losses.items():
            # Compute partial derivative of the less with respect to the input.
            if name == 'motion_recon':
                grad, = autograd.grad(loss, [motion_recon], retain_graph=True, allow_unused=True)
            else:
                grad, = autograd.grad(loss, [music_recon], retain_graph=True, allow_unused=True)
            if self.per_batch_item:
                # We do not average the gradient over the batch dimension.
                dims = tuple(range(1, grad.dim()))
                norm = grad.norm(dim=dims, p=2).mean()
        ...
        music_out_grad = torch.zeros_like(music_recon)
        motion_out_grad = torch.zeros_like(motion_recon)
        effective_loss = torch.tensor(0., device=music_recon.device, dtype=music_recon.dtype)
        for name, avg_norm in avg_norms.items():
            if self.balance_grads:
                # g_balanced = g / avg(||g||) * total_norm * desired_ratio
                scale = desired_ratios[name] * self.total_norm / (self.epsilon + avg_norm)
                ...
                scale = self.weights[name]
            if name == 'motion_recon':
                motion_out_grad.add_(grads[name], alpha=scale)
            else:
                music_out_grad.add_(grads[name], alpha=scale)
            effective_loss += scale * losses[name].detach()
        # Send the computed partial derivative with respect to the output of the model to the model.
        # input.backward(out_grad)
        return effective_loss, music_out_grad, motion_out_grad

...
from mmldm.audio.xformers_ import ops
...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def _get_mask(self, current_steps: int, device: torch.device, dtype: torch.dtype):
        ...
        if self.memory_efficient:
            from mmldm.audio.xformers_.ops import LowerTriangularMask
            if current_steps == 1:
...
def _verify_xformers_memory_efficient_compat():
    try:
        from mmldm.audio.xformers_.ops import memory_efficient_attention, LowerTriangularMask  # noqa
    except ImportError:
        raise ImportError(

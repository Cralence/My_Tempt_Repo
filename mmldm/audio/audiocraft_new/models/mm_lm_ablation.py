...
class LMModel_Ablation1(StreamingModule):
...
class LMModel_Ablation2(StreamingModule):
    """Transformer-based language model on multiple streams of codes.
    ...
    def forward(self,
        ...
        music_logits = torch.stack([self.linears[k](out[:, :S // 2]) for k in range(K)], dim=1)  # [B, K, S/2, card]
        motion_logits = torch.stack([self.linears[k](out[:, S // 2:]) for k in range(K)], dim=1)   # [B, K, S/2, card]
...
class LMModel_Ablation3(StreamingModule):
    """Transformer-based language model on multiple streams of codes.
    ...
    @property
    def music_start_token_id(self) -> int:
        return self.card + 1

    @property
    def motion_start_token_id(self) -> int:
        return self.card + 2
    ...
    def compute_predictions(
        ...
        logits = self(sequence_codes, conditions, condition_tensors)  # [B, K, S, card]
        # map back the logits on pattern sequence to logits on original codes: [B, K, S, card] -> [B, K, T, card]
        # and provide the corresponding mask over invalid positions of tokens
    ...
    def generate(self,
                 ...
                 remove_prompts: bool = False,
                 check: bool = True,
                 callback: tp.Optional[tp.Callable[[int, int], None]] = None) -> tp.Tuple[torch.LongTensor, torch.LongTensor]:
        ...
        pattern = self.pattern_provider.get_pattern(max_gen_len * 2 + 2)
        # this token is used as default value for codes that are not generated yet
        unknown_token = -1

        # we generate codes up to the max_gen_len that will be mapped to the pattern sequence
        gen_codes = torch.full((B, K, max_gen_len * 2 + 2), unknown_token, dtype=torch.long, device=device)
        gen_codes[:, :, 0] = self.music_start_token_id
        gen_codes[:, :, max_gen_len + 1] = self.motion_start_token_id
        # filling the gen_codes with the prompt if needed
        gen_codes[..., :start_offset] = prompt
        # create the gen_sequence with proper interleaving from the pattern: [B, K, S]
        gen_sequence, indexes, mask = pattern.build_pattern_sequence(gen_codes, self.special_token_id)   # gen_sequence: padded with self.special_token_id
        # retrieve the start_offset in the sequence:
        # it is the first sequence step that contains the `start_offset` timestep
        start_offset_sequence = pattern.get_first_step_with_timesteps(start_offset)
        assert start_offset_sequence is not None
        ...
        assert (out_codes[:, :, 0] == self.music_start_token_id).all()
        assert (out_codes[:, :, max_gen_len + 1] == self.motion_start_token_id).all()
        music_out_codes = out_codes[:, :, 1: max_gen_len + 1]
        motion_out_codes = out_codes[:, :, max_gen_len + 2:]
...
class LMModel_Ablation4(StreamingModule):
    def __init__(self, pattern_provider: CodebooksPatternProvider, condition_provider: ConditioningProvider,
                 fuser: ConditionFuser, n_q: int = 8, card: int = 1024, dim: int = 128, num_heads: int = 8,
                 hidden_scale: int = 4, norm: str = 'layer_norm', norm_first: bool = False,
                 emb_lr: tp.Optional[float] = None, bias_proj: bool = True,
    ...
    def generate(self,
        ...
        pattern = self.pattern_provider.get_pattern(max_gen_len * 2)
        # this token is used as default value for codes that are not generated yet
        unknown_token = -1

        # we generate codes up to the max_gen_len that will be mapped to the pattern sequence
        gen_codes = torch.full((B, K, max_gen_len * 2), unknown_token, dtype=torch.long, device=device)
        # filling the gen_codes with the prompt if needed
        gen_codes[..., :start_offset] = prompt
        # create the gen_sequence with proper interleaving from the pattern: [B, K, S]
        gen_sequence, indexes, mask = pattern.build_pattern_sequence(gen_codes, self.special_token_id)   # gen_sequence: padded with self.special_token_id
        ...
        # get back the codes, trimming the prompt if needed and cutting potentially incomplete timesteps
        out_codes, out_indexes, out_mask = pattern.revert_pattern_sequence(gen_sequence, special_token=unknown_token)
        music_out_codes = out_codes[:, :, ::2]
        motion_out_codes = out_codes[:, :, 1::2]
...
class LMModel_Ablation5(StreamingModule):
    """Transformer-based language model on multiple streams of codes.
    ...
    def __init__(self, pattern_provider: CodebooksPatternProvider, condition_provider: ConditioningProvider,
        ...
        self.emb = nn.ModuleList([ScaledEmbedding(embed_dim, dim, lr=emb_lr) for _ in range(n_q)])
        self.motion_emb = ScaledEmbedding(embed_dim, dim, lr=emb_lr)
    ...
    def forward(self,
        ...
        music_sequence = sequence[:, :, :S // 2]
        motion_sequence = sequence[:, :, S // 2:]

        input_music = sum([self.emb[k](music_sequence[:, k]) for k in range(K)])  # b, s/2, dim
        input_motion = self.motion_emb(motion_sequence[:, 0])  # b, s/2, dim
        assert input_music.shape == input_motion.shape, f'{input_music.shape} != {input_motion.shape}'
        input_ = torch.cat([input_music, input_motion], dim=1)  # b, s, dim
    ...
    def compute_predictions(
        ...
        """
        # prepare input sequence
        assert music_codes.shape == motion_codes.shape
        B, K, T_music = music_codes.shape
        T_motion = motion_codes.shape[-1]
        assert T_music == T_motion
        music_codes = music_codes.contiguous()
    ...
    def _sample_next_token(self,
        ...
        Returns:
            next_token (torch.Tensor): Next token tensor of shape [B, K, 1].
        """
        assert music_sequence.shape == motion_sequence.shape
        B = music_sequence.shape[0]
        cfg_coef = self.cfg_coef if cfg_coef is None else cfg_coef
        two_step_cfg = self.two_step_cfg if two_step_cfg is None else two_step_cfg
    ...
    def generate(
        ...
        motion_gen_sequence, _, motion_mask = pattern.build_pattern_sequence(motion_gen_codes, self.special_token_id)   # gen_sequence: padded with self.motion_special_token_id
        # retrieve the start_offset in the sequence:
        # it is the first sequence step that contains the `start_offset` timestep
        start_offset_sequence = pattern.get_first_step_with_timesteps(start_offset)
        assert start_offset_sequence is not None

...
def load_mm_lm_ablation_1_model(file_or_url_or_id: tp.Union[Path, str], device='cpu', cache_dir: tp.Optional[str] = None, use_autocast: bool = True):
    pkg = load_lm_model_ckpt(file_or_url_or_id, cache_dir=cache_dir)
    cfg = OmegaConf.create(pkg['xp.cfg'])
    cfg.device = str(device)
    if cfg.device == 'cpu' or not use_autocast:
    ...
    model = builders.get_ablation_1_lm_model(cfg)
...
def load_mm_lm_ablation_2_model(file_or_url_or_id: tp.Union[Path, str], device='cpu', cache_dir: tp.Optional[str] = None, use_autocast: bool = True):
    pkg = load_lm_model_ckpt(file_or_url_or_id, cache_dir=cache_dir)
    cfg = OmegaConf.create(pkg['xp.cfg'])
    cfg.device = str(device)
    if cfg.device == 'cpu' or not use_autocast:
    ...
    model = builders.get_ablation_2_lm_model(cfg)
...
def load_mm_lm_ablation_3_model(file_or_url_or_id: tp.Union[Path, str], device='cpu', cache_dir: tp.Optional[str] = None, use_autocast: bool = True):
    pkg = load_lm_model_ckpt(file_or_url_or_id, cache_dir=cache_dir)
    cfg = OmegaConf.create(pkg['xp.cfg'])
    cfg.device = str(device)
    if cfg.device == 'cpu' or not use_autocast:
    ...
    model = builders.get_ablation_3_lm_model(cfg)
...
def load_mm_lm_ablation_4_model(file_or_url_or_id: tp.Union[Path, str], device='cpu', cache_dir: tp.Optional[str] = None, use_autocast: bool = True):
    pkg = load_lm_model_ckpt(file_or_url_or_id, cache_dir=cache_dir)
    cfg = OmegaConf.create(pkg['xp.cfg'])
    cfg.device = str(device)
    if cfg.device == 'cpu' or not use_autocast:
    ...
    model = builders.get_ablation_4_lm_model(cfg)
...
def load_mm_lm_ablation_5_model(file_or_url_or_id: tp.Union[Path, str], device='cpu', cache_dir: tp.Optional[str] = None, use_autocast: bool = True):
    pkg = load_lm_model_ckpt(file_or_url_or_id, cache_dir=cache_dir)
    cfg = OmegaConf.create(pkg['xp.cfg'])
    cfg.device = str(device)
    if cfg.device == 'cpu' or not use_autocast:
    ...
    model = builders.get_ablation_5_lm_model(cfg)

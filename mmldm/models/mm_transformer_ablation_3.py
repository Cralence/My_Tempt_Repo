...
from mmldm.audio.audiocraft_new.models.mm_lm_ablation import LMModel_Ablation3
from mmldm.audio.audiocraft_new.models.builders import get_debug_lm_model
from mmldm.audio.audiocraft_new.models.loaders import load_mm_lm_ablation_3_model
...
class MM_Transformer(pl.LightningModule):
    def __init__(self,
        ...
        self.name = name
        self.model: LMModel_Ablation3 = self.get_pretrained_lm(name, use_autocast=autocast)
    ...
    def get_pretrained_lm(self, name: str = 'facebook/musicgen-melody', device=None, use_autocast=True):
        ...
        lm = load_mm_lm_ablation_3_model(name, device=device, use_autocast=use_autocast)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.stage == 'train_music_motion':
            ...
            B, K, T_music = music_code.shape
            T_motion = motion_code.shape[-1]
            assert T_music == T_motion
            music_codes = music_code.contiguous()
            motion_codes = motion_code.contiguous()
            music_start_token = torch.full((B, K, 1), self.model.music_start_token_id).to(self.device)
            motion_start_token = torch.full((B, K, 1), self.model.motion_start_token_id).to(self.device)
            codes = torch.cat((music_start_token, music_codes, motion_start_token, motion_codes), dim=-1)

            with self.autocast:
                output = self.model.compute_predictions(codes, [], condition_tensors=text_condition)
                logits = output.logits
                mask = output.mask

                mask[:, :, 0] = False
                mask[:, :, T_music + 1] = False

                loss, loss_per_codebook = self._compute_cross_entropy(logits, codes, mask)
                total_loss = loss

            self.log("train/loss", total_loss, prog_bar=True, logger=True, on_step=True, on_epoch=False)

            log_dict = {}
            for k in range(len(loss_per_codebook)):
                log_dict[f'train/ce_q{k + 1}'] = loss_per_codebook[k]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.stage == 'train_music_motion':
            ...
            log_dict = {}
            for k in range(len(loss_per_codebook)):
                log_dict[f'val/ce_q{k + 1}'] = loss_per_codebook[k]

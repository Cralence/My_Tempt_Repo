...
class MusicMotionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        music_path = pjoin(self.music_dir, f'{music_id}.wav')
        waveform, sr = librosa.load(music_path, sr=self.vqvae_sr)
        waveform = waveform[None, ...]
        segment_length = int(self.music_target_length * 640)

        feature_dict = torch.load(pjoin(self.feature_dir, f'{music_id}.pth'))

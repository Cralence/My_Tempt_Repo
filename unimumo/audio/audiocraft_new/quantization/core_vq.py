...
#import flashy
from unimumo.packages.flashy.distrib import broadcast_tensors
import torch
...
class VectorQuantization(nn.Module):
    ...
    def forward(self, x, freeze_codebook=False):
        ...
        x = self.project_in(x)
        quantize, embed_ind = self._codebook(x, freeze_codebook=freeze_codebook)
...
class ResidualVectorQuantization(nn.Module):
    ...
    def forward(self, x, n_q: tp.Optional[int] = None, freeze_codebook=False):
        quantized_out = 0.0
        residual = x
        ...
        for i, layer in enumerate(self.layers[:n_q]):
            quantized, indices, loss = layer(residual, freeze_codebook=freeze_codebook)
